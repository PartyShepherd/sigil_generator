...
def draw_rose_sigil(word="JAMES"):
    ...
    all_positions["H"] = (
        -np.cos(h_index * (2 * np.pi / len(ordered_outer_letters))) * 2,
    ...
    all_positions["M"] = (-0.7, 0.0)
    all_positions["Sh"] = (0.7, 0.0)
    ...
    for char in word:
        if char in letter_mapping:
            ...
            if prev_pos:
                ax.plot([prev_pos[0], x], [prev_pos[1], y],
                        color=prev_color, lw=2)

            if (x, y) in seen_positions:
                ax.add_patch(Circle((x, y), 0.2, fill=False,
    ...
    if first_pos:
        ax.add_patch(Circle(first_pos, 0.2, fill=False,
...
# ✅ ONLY CHANGE: Railway-compatible port binding
import os
